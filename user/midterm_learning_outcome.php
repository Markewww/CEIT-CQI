...
while ($row = $result->fetch_assoc()) {
    $existingMLOs[intval($row['QN'])] = $row;
}
$stmt->close();
...
if (!$total_q) {
    $count_qn = "SELECT MAX(QN) FROM mlo WHERE schedule_id = ?";
    ...
    <div id="mainContent" class="container p-4">
        ...
        <form action="" method="POST">
            <div class="table-container">
                ...
                <a onclick="updateMLOS(<?php echo htmlspecialchars($schedule_id); ?>);" class="btn btn-sm btn-success mb-1">
                    Summarize
                </a>
                </div>
            </div>
...
function updateMLO(id, column, value) {
    $.ajax({
        url: './operations/update_mlo.php', // Path from JS file to PHP script
        type: 'POST',
