...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['id']) && isset($_POST['column']) && isset($_POST['value'])) {
        ...
        $sql = "UPDATE flo SET $column = ? WHERE id = ?";
        ...
        $queryILO = "SELECT DISTINCT ILO FROM flo WHERE schedule_id = ?";
        ...
        if (!empty($ilo_numbers)) {
            ...
            $deleteMissingILO = "DELETE FROM flos WHERE schedule_id = ? AND ILO NOT IN ($placeholders)";
            $stmtDelete = $conn->prepare($deleteMissingILO);
            $stmtDelete->bind_param("i" . $types, $schedule_id, ...$ilo_numbers);
            $stmtDelete->execute();
            $stmtDelete->close();

            $deleteMissingMaps = "DELETE FROM faps WHERE schedule_id = ? AND ILO NOT IN ($placeholders)";
            ...
            echo "All FLOS and FAPS records deleted.";
            exit();
        }
        ...
        foreach ($ilo_numbers as $ilo) {
            ...
            $querySumATT = "SELECT COALESCE(SUM(ATT), 0), COALESCE(COUNT(*), 1), COALESCE(MAX(TAR), 0) FROM flo WHERE schedule_id = ? AND ILO = ?";
            ...
            $checkILO = "SELECT COUNT(*) FROM flos WHERE schedule_id = ? AND ILO = ?";
            ...
            if ($count > 0) {
                $updateILO = "UPDATE flos SET ATT = ?, TAR = ?, RM = ? WHERE schedule_id = ? AND ILO = ?";
                $stmtUpdate = $conn->prepare($updateILO);
                $stmtUpdate->bind_param("ddsii", $ilo_att, $target, $remarks, $schedule_id, $ilo);
                $stmtUpdate->execute();
                $stmtUpdate->close();
            } else {
                $insertILO = "INSERT INTO flos (schedule_id, ILO, ATT, TAR, RM) VALUES (?, ?, ?, ?, ?)";
            ...
            $apsQuery = "SELECT GROUP_CONCAT(DISTINCT AP ORDER BY AP ASC SEPARATOR ', ') AS summarized_APS FROM flo WHERE schedule_id = ? AND ILO = ?";
            ...
            $examTypeQuery = "SELECT DISTINCT exam_type FROM flo WHERE schedule_id = ?";
            ...
            $exam_type = $examTypeResult['exam_type'] ?? 'Finals';
            $stmtExamType->close();

            $checkQuery = "SELECT ID FROM faps WHERE schedule_id = ? AND ILO = ?";
            ...
            if ($exists) {
                $updateMapsQuery = "UPDATE faps SET APS = ?, exam_type = ? WHERE schedule_id = ? AND ILO = ?";
                $stmt = $conn->prepare($updateMapsQuery);
                $stmt->bind_param("ssii", $summarized_APS, $exam_type, $schedule_id, $ilo);
            } else {
                $insertMapsQuery = "INSERT INTO faps (schedule_id, exam_type, ILO, APS) VALUES (?, ?, ?, ?)";
        ...
        echo "FLOS and FAPS successfully updated.";
    } else {
