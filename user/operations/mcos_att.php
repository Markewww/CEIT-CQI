...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($sumResult->num_rows > 0) {
        while ($row = $sumResult->fetch_assoc()) {
            ...
            $countCOQuery = "SELECT COUNT(*) as countCO FROM mlos WHERE schedule_id = ? AND CO = ?";
            ...
            $checkQuery = "SELECT COUNT(*) FROM mcos WHERE schedule_id = ? AND CO = ?";
            ...
            if ($count > 0) {
                ...
                $insertQuery = "INSERT INTO mcos (schedule_id, CO, ATT, TAR, RM) 
                                VALUES (?, ?, ?, ?, ?)";
    ...
    if (!empty($existingCOs)) {
        ...
        $deleteQuery = "DELETE FROM mcos WHERE schedule_id = ? AND CO NOT IN ($placeholders)";
        ...
        $deleteAllQuery = "DELETE FROM mcos WHERE schedule_id = ?";
