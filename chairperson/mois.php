...
if (isset($_GET['subject_code'])) {
    ...
    if (!empty($schedule_ids)) {
        // Fetch ILOs and attainment from the mlo table for the collected schedule IDs
        $placeholders = implode(',', array_fill(0, count($schedule_ids), '?'));
        $sql = "SELECT schedule_id, ilo, ATT FROM mlo WHERE schedule_id IN ($placeholders)";
        $stmt = $conn->prepare($sql);
        ...
        // Group by ILO and calculate average ATT
        $ilo_attainment = [];
        foreach ($data as $row) {
            $ilo = $row['ilo'];
            $att = floatval($row['ATT']); // ensure numeric

            if (!isset($ilo_attainment[$ilo])) {
                $ilo_attainment[$ilo] = ['sum' => 0, 'count' => 0];
            }

            $ilo_attainment[$ilo]['sum'] += $att;
            $ilo_attainment[$ilo]['count']++;
        }

        // Calculate averages
        $averages = [];
        foreach ($ilo_attainment as $ilo => $values) {
            $averages[$ilo] = $values['sum'] / $values['count'];
        }
...
<head>
    ...
    <title>MOIS - Subject Details</title>
    ...
    <div class="d-flex">
        ...
        <div id="mainContent" class="container p-4">
            <div class="table-container">
                ...
                <?php else: ?>
                    <h5>Midterm Overall Intended Learning Outcome Summary</h5>
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <?php if (!empty($averages)): ?>
                                <?php foreach ($averages as $ilo => $avg_att): ?>
                                    <tr>
                                        <td><?php echo htmlspecialchars($ilo, ENT_QUOTES, 'UTF-8'); ?></td>
