...
const DepartmentCreateModal: React.FC<DepartmentCreateModalProps> = ({ onClose, onDepartmentCreated }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.code.trim() || !formData.name.trim()) {
            setMessage({ type: 'error', text: "All fields are required to process entry." });
            return;
        }
        ...
        try {
            ...
            if (result.status === "success") {
                setMessage({ type: 'success', text: "New department branch registered successfully." });
                setTimeout(() => {
                    onDepartmentCreated();
                    onClose();
                }, 1000);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-xl max-w-xl w-full overflow-hidden max-h-[90vh] flex flex-col transform scale-up-in">
                ...
                <div className="p-6 bg-slate-50 border-b border-slate-100 flex justify-between items-center shrink-0">
                    <div>
                        <h3 className="text-lg font-bold text-slate-900 font-montserrat">Add New Department</h3>
                        <p className="text-xs text-slate-500 mt-0.5">Register a brand new official institutional branch into the system infrastructure core.</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <input
                            ...
                            required
                        />
                        <span className="text-[10px] text-slate-400 font-medium">Use unique short institutional acronym code indicators.</span>
                    </div>
